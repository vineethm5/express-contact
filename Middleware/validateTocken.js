...
const validatejwt= asynchandler( async(req,res,next)=>{
    let accesstocken;
    let authheader = req.headers.authorization || req.headers.Authorization; //we will get access token in header auth field as Bearer
    ...
    if(authheader && authheader.startsWith("Bearer"))
    {
        ...
        accesstocken = authheader.split(" ")[1]; // we fetching token by its position 
